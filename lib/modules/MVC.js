...
MVC.prototype.widgetAction=function(name,params)
{
  return function(req,res,next){
    var runtime=this.web.install(req,res,next);
    runtime.widget(name,params,function(err,data){
      if (data==false)
      {
	return next(err);
      }
...
MVC.prototype.registerWidget=function(name,func,settings)
{
  settings=settings?settings:{};
  settings['func']=func;
  this.widgets[name]=settings;
}
...
var widget=function(name,params,callback)
{
  var keyword;
  var runtime=this;
  ...
  var widgetSettings=this.web.MVC.widgets[name]
  if (widgetSettings==null || widgetSettings.func==null){return callback(null,null)}  
  var data={};
  data.runtime=this;
  data.widgetName=name;
  data.params=params;
  if (runtime.widgets[keyword]==null)
  {
    return widgetSettings.func(data,function(err,content){
      runtime.widgets[keyword]=content
      return callback(err,content);
    });
  }
  callback(null,runtime.widgets[keyword]);
}
...
var fetchData=function(name,callback)
{
  ...
  if ( this.dataProviders[keyword]===undefined)
  {
    ...
    if (func!=null)
    {
      return func({runtime:this,params:params},afterFetch.bind(this,keyword,callback));
    }
  }
