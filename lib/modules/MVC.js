...
{
  this.moduleName='MVC';
  this.viewpath=web.settings.viewpath;
...
MVC.prototype.getSe=function(name)
{
  if (name==null){name='sequelize';}
  if (this._sequelizes[name]==null)
  {
    var db=this.web.settings.db;
    this._sequelizes[name]=new Sequelize(db[name].url,db[name].options);
  }
  return this._sequelizes[name];
}
...
var defualtDataValidator=function(data,runtime,callback)
{
  return callback(null,data!=null&& data!=false);
}
MVC.prototype.isData=function(name,path,Validator)
{
  return function(runtime,callback){
    runtime.fetchData(name,function(err,data){
      if (err){return callback(err,data)}
      var realdata=(path==null)?data:_.get(data,path);
      return defualtDataValidator(realdata,runtime,callback);
    });
  }
}
